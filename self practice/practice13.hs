...
digitsNeg :: Parser Char Int 
digitsNeg = (negate <$> (char '-' *> digits))  <|> ( digits) 

tstNeg = fmap (*2) <$> runParser digitsNeg "-123asd"

data Op = Add | Sub | Mul | Div 
operator :: Parser Char Op 
operator = Parser f where 
    f (x : xs)  |x == '+' = MyJust (xs, Add)
                |x == '-' = MyJust (xs, Sub)
                |x == '*' = MyJust (xs, Mul)
                |x == '/' = MyJust (xs, Div) 
    f           _ = MyNothing 
...
calculate  x Add  y = x + y
calculate  x Sub  y = x - y 
calculate  x Div  y = x `div` y 
calculate  x Mul   y = x * y 

expr :: Parser Char Int 
expr = calculate <$> digits <*> operator <*> digits

tstexpr=runParser expr "12-4abc"